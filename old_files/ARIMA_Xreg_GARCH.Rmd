...
Here is the plot of real data vs. Garch model forecast (differenced). Although the model cannot reflect as much as the votality as of the real data, but we can see when the real data has larger votality, the model also shows larger votality.

```{r}
pred.Garch <- suppressWarnings(predict(garch1.1, n.ahead=251))
pred.Garch1 <- as.list(pred.Garch["meanForecast"])$meanForecast
plot(1:2263, Nas.train.diff, type="l", main = "Real data vs. predicted value (Differenced)")
...
lines(2265:2515, pred.Garch2, col=2)
lines(2265:2515, pred.Auto$mean, col=4)
lines(2265:2515, pred.Xreg$mean, col="orange")
```
