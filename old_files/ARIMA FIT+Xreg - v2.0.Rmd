...
```{r}
Nas2 <- diff(Nas)
ARMA <- auto.arima(Nas2) # ARMA(2,2)
acf2(residuals(ARMA)^2)
```
Try garch(1,2): The highest spikes are acf: 1 and pacf: 1 or 2
```{r}
garch <- garchFit(~arma(2,2)+garch(1,1), Nas2)
summary(garch)
# predict(garch, n.ahead = 251)
# plot(pred2)
```
...
GARCH.Auto = ugarchfit(data = Nas2, spec = spec)
GARCH.Auto
GARCH.Xreg = ugarchfit(data = Nas2, spec = spec2)
GARCH.Xreg
```

```{r}
pred2 <- ugarchforecast(GARCH.Auto,n.ahead=251, data = Nas2[1:2644, ,drop=FALSE])
#plot(pred2)
pred2
